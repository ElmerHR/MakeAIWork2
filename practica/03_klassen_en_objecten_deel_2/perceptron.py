...
class Perceptron:
    ...
    def train(self, X, y, epochs=100, learningRate=0.1):
        ...
        for epoch in epochs:
            ...
            for inputVector, label in zip(X, y):
                ...
                # update weight and b
                if error != 0:
                    for idx, x_i in enumerate(inputVector):
                        deltaWeight = learningRate * (self.weightVector + (error * x_i))
                        ...
                        logging.debug(f"updatedWeights : {self.weightVector}")
                        logging.debug(f"updatedBias : {self.bias}")
                print()
    ...
    def predict(self, inputVector):
        ...
        HINT: use np.dot()
        """
        logging.debug(f"Weights : {self.weightVector}")
