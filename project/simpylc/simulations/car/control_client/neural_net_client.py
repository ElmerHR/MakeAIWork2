...
class NeuralNetClient:
    ...
    def inference(self):
        ...
        file_lidar = open('models/nn_lidar_layers_4_64_64_128_epochs_500_lr_0001_batchsize_60_batchnorm_relu.pickle', 'rb')
        file_sonar = open('models/nn_sonar_layers_5_16_64_256_64_epochs_300_lr_0001_batchsize_70_batchnorm_relu.pickle', 'rb')

        # dump information to that file
        model_lidar = pickle.load(file_lidar)
        model_sonar = pickle.load(file_sonar)
        ...
        if hasattr (self, 'lidarDistances'):
            ...
            with torch.no_grad():
                # print(sample[:-1])
                self.steeringAngle = model_lidar(torch.Tensor(sample[:-1]).to(torch.float32).reshape(1, -1).to(DEVICE)).item()
        else:
            ...
            with torch.no_grad():
                print(sample[:-1])
                self.steeringAngle = model_sonar(torch.Tensor(sample[:-1]).to(torch.float32).reshape(1, -1).to(DEVICE)).item()
        self.targetVelocity = pm.getTargetVelocity (self.steeringAngle)
